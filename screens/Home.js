		...
		<View style={tw` w-full h-full bg-white justify-between`}>
			...
			<View style={[styles.search, tw`py-1 shadow-sm px-3 bg-slate-100`]}>
				...
				>
					...
					{!showSearch && (
						<>
							<Text style={tw`text-lg ml-1 font-semibold`}>Photo</Text>
							<Text style={tw`text-lg text-blue-700 font-semibold`}>
								Corner
							</Text>
						</>
					)}
				...
				<View style={tw`flex-row items-center bg-white px-2 rounded-3xl`}>
					...
					>
						<FontAwesome
							name="search"
							color="blue"
							style={tw`text-xl rounded-xl px-2 py-1`}
						/>
					</Pressable>
					{showSearch && (
						<KeyboardAvoidingView style={tw`w-10/12 flex-row `}>
							<TextInput
								value={query}
			...
			>
				{/* <Stories /> */}
				<View
					style={[tw`flex-row mt-3 items-center mx-auto w-full justify-center`]}
				>
					<Image
						style={tw`h-10 w-10 rounded-full border-2 border-blue-400`}
					...
					>
						<Text style={tw`text-slate-600`}>
							{user.username}, Tap to Create new Post
						</Text>
					</TouchableOpacity>
				</View>
				{curPosts.map((post) => (
